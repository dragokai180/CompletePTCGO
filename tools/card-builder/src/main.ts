...
async function refreshReprintCandidates(): Promise<void> {
  ...
  const set = (draft.spiritSetCode || draft.set).trim().toUpperCase();
  ...
  try {
    ...
    reprintCandidates = candidates.map(candidate => ({
      ...
      imageUrl: cardImageUrl(
        cardsBySet.get(candidate.set)?.find((card: { localId: string; image?: string }) => card.localId === candidate.setNumber)?.image,
        'high'
      ),
    }));
    const visible = matchingReprintCandidates();
  ...
  const heroActions =
    view === 'browse'
      ...
      : `<button type="button" data-action="show-browse">← Browse cards</button>
         ...
         <button type="button" data-action="save" ${canSave ? '' : 'disabled'}>${saveAsReprint && selectedReprint ? 'Save reprint stub' : 'Save to spirit scripts'}</button>
    ...
    view === 'browse'
      ...
      <form class="form" id="card-form">
        ...
        <section class="section identity-section">
          <h2>Identity</h2>
          <div class="grid-3">
            <label>Script stem<input data-root="className" value="${escapeAttr(draft.className)}" placeholder="${escapeAttr(computedClassName())}" /></label>
            <label>Kind
              <select data-root="extends">
                <option value="PokemonCard" ${draft.extends === 'PokemonCard' ? 'selected' : ''}>Pokémon</option>
                <option value="TrainerCard" ${draft.extends === 'TrainerCard' ? 'selected' : ''}>Trainer</option>
                <option value="EnergyCard" ${draft.extends === 'EnergyCard' ? 'selected' : ''}>Energy</option>
              </select>
            </label>
            <label>Name<input data-root="name" value="${escapeAttr(draft.name)}" /></label>
          </div>
          <div class="grid-3">
            <label>Spirit set<code> </code><input data-root="spiritSetCode" value="${escapeAttr(draft.spiritSetCode)}" placeholder="SV09" /></label>
          ...
          <div class="grid-3">
            <label>Catalog id<input data-root="catalogId" value="${escapeAttr(draft.catalogId)}" placeholder="sv9-22" /></label>
            <label>Rarity<input data-root="rarity" value="${escapeAttr(draft.rarity)}" placeholder="Common" /></label>
            <label>PTCGO / abbr<input data-root="set" value="${escapeAttr(draft.set)}" placeholder="JTG" /></label>
          </div>
          <div class="grid-2">
            <label>Output file<input readonly value="${escapeAttr(scriptFileName(draft))}" /></label>
            <label>Image URL<input data-root="imageUrl" value="${escapeAttr(draft.imageUrl)}" placeholder="https://images.pokemontcg.io/…" /></label>
          </div>
          ${
            draft.extends === 'PokemonCard'
              ? renderPokemonFields()
...
async function handleAction(action: string | null, btn: HTMLButtonElement) {
  switch (action) {
    ...
    case 'save': {
      syncAllInputs();
      const selected = selectedReprintCandidate();
      const spiritSet = resolveSpiritSetCode(draft) || draft.spiritSetCode || draft.set;
      let sourceToSave = outputCode;
      const reprint = saveAsReprint && selected
        ? {
            fileName: selected.fileName || `${selected.className}.py`,
      ...
      const save = async (overwrite: boolean) => {
        const response = await fetch('/save-card', {
          ...
          body: JSON.stringify({
            ...
            source: sourceToSave,
            overwrite,
            imageUrl: draft.imageUrl || sourceMeta?.imageUrl || '',
            downloadImage: true,
            ...(reprint ? { reprint } : {}),
          }),
        });
        ...
        if (response.status === 409 && !overwrite) {
          if (window.confirm('That card script already exists. Overwrite it?')) {
            await save(true);
          }
          return;
        }
        if (!response.ok) throw new Error(result.error || `Save failed (${response.status})`);
        statusMessage = result.message || 'Saved to spirit scripts.';
        if (result.warnings?.length) {
          statusMessage += ` — ${result.warnings.join(' ')}`;
        }
        outputError = '';
        if (reprint) outputCode = sourceToSave;
        render();
      };
